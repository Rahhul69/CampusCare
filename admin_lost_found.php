...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_item"])) {
    try {
        ...
        if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
            ...
            // Update the insert statement in your form submission handling
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                // Change the number of parameter placeholders to match the values
                $stmt = $conn->prepare("INSERT INTO lost_items (item_name, description, category, image_path, date_found, location_found, status, reported_by) VALUES (?, ?, ?, ?, ?, ?, 'unclaimed', 0)");
                $stmt->bind_param("ssssss", $item_name, $description, $category, $target_file, $date_found, $location_found);
                
                if (!$stmt->execute()) {
                    throw new Exception("Failed to save item details: " . $conn->error);
                }
                                ...
                                <div class="action-buttons">
                                    <form method="POST" style="display: inline;">
                                        ...
                                        <input type="hidden" name="status" 
                                               value="<?= $item['status'] === 'unclaimed' ? 'claimed' : 'unclaimed' ?>">
                                        <button type="submit" name="update_status" 
                                                class="btn <?= $item['status'] === 'unclaimed' ? 'btn-success' : 'btn-warning' ?>">
